...
<div class="container">
    ...
    <table class="table table-responsive" id="tableLoan">
        ...
        <%                    try {
                ...
                createCriteria.addOrder(Order.desc("issueDate"));
                List<Loan> list = createCriteria.list();
                for (Loan loan : list) {
                    Iterator it = loan.getMembers().getCenter().getCenterHasStaffs().iterator();

                    while (it.hasNext()) {
                        CenterHasStaff next = (CenterHasStaff) it.next();

                        if (next.getStaff().getNic().equals(session.getAttribute("nic").toString())) {
        ...
        <td> <div class="dropdown">
                <button type="button" class="btn btn-primary dropdown-toggle men" data-toggle="dropdown" idloan = <%= loan.getIdLoan()%>>
                    Action
                </button>
